...
def run():
    ...
    # ------------------------------
        ...
        # If parquet refs already exist locally, open them
        # (fast path). This part is commented since on the fly
        # transformation is faster than loading the parquet
        # file in actual config (check why at some point).
        # Loading from local parquet is also slower than
        # loading json and convert the path on the fly...
        ...
        # Else: fetch JSON, patch refs to https, open, AND write parquet refs cache
            ...
            print(
                f"ℹ️ No local parquet refs found at "
                f"./{OUT_PARQUET} -> creating them from JSON"
            )
    ...
    zeta0_roi = apply_polygon_mask(
        zeta0, poly
    )  # returns DataArray masked outside polygon
    zeta_mask_2d = zeta0_roi.notnull()  # True where water points exist inside polygon

    # 3) Convert mask to a 1D point selector (integer
    # positions), using stack without MultiIndex
    ...
    ds_geom = (
        ds.isel(time_counter=0)
        .stack(point=("y_rho", "x_rho"), create_index=False)
        .isel(point=point_idx)
    )
    ...
    for t0 in range(0, nt, block):
        ...
        ds_in = ds_blk.stack(point=("y_rho", "x_rho"), create_index=False).isel(
            point=point_idx
        )

        # --- Regrid each variable (keeps graph smaller
        # than applying to whole Dataset) ---
        out_vars = {}
        ...
        ds_aligned = ds_hp.reindex(cell_ids=target_ids, fill_value=np.nan).chunk(
            {"time_counter": min(block, time_chunk_size), "cell_ids": chunk_size}
        )
